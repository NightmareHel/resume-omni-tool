    ...
    <div className="min-h-screen bg-zinc-950 text-zinc-100">
      <header className="border-b border-zinc-800 bg-zinc-950/80 backdrop-blur sticky top-0 z-10">
        <div className="max-w-3xl mx-auto px-4 h-14 flex items-center justify-between">
          <span className="font-semibold text-zinc-100 tracking-tight">Resume Analyzer</span>
          {step > 1 && (
          ...
          <section className="flex flex-col gap-6">
            ...
            <div
              ...
              className={`relative rounded-xl border-2 border-dashed p-8 text-center transition-all cursor-pointer ${
                dragActive ? 'border-emerald-500 bg-emerald-500/5' : fileName ? 'border-emerald-500/50 bg-emerald-500/5 cursor-default' : 'border-zinc-700 hover:border-zinc-500 hover:bg-zinc-900/50'
              }`}
            >
              ...
              {fileName ? (
                <div className="flex flex-col items-center gap-2">
                  ...
                  <button onClick={(e) => { e.stopPropagation(); clearFile(); }} className="text-xs text-zinc-500 hover:text-zinc-300 underline">Remove</button>
                </div>
              ) : (
                <div className="flex flex-col items-center gap-2">
                  ...
                  <p className="text-sm text-zinc-400"><span className="text-zinc-200 font-medium">Drop a file</span> or click to browse</p>
            ...
            <textarea
              ...
              rows={10}
              className="w-full rounded-lg bg-zinc-900 border border-zinc-800 text-sm text-zinc-300 placeholder-zinc-600 p-4 resize-y focus:outline-none focus:border-emerald-500 transition-colors font-mono leading-relaxed"
            />
            <button
              onClick={handleParseResume}
              disabled={loading}
              className="self-start px-6 py-2.5 rounded-lg bg-emerald-700 text-white text-sm font-semibold hover:bg-emerald-600 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
            >
          ...
          <section className="flex flex-col gap-8">
            ...
            <div className="border-t border-zinc-800 pt-8 flex flex-col gap-4">
              ...
              <textarea
                ...
                rows={10}
                className="w-full rounded-lg bg-zinc-900 border border-zinc-800 text-sm text-zinc-300 placeholder-zinc-600 p-4 resize-y focus:outline-none focus:border-emerald-500 transition-colors leading-relaxed"
              />
              <div className="flex gap-3">
                <button onClick={handleAnalyzeJD} disabled={loading} className="px-6 py-2.5 rounded-lg bg-emerald-700 text-white text-sm font-semibold hover:bg-emerald-600 disabled:opacity-50 disabled:cursor-not-allowed transition-colors">
                  {loading ? 'Analyzing...' : 'Analyze Keywords'}
                </button>
                <button onClick={() => setStep(3)} className="px-6 py-2.5 rounded-lg bg-zinc-800 text-zinc-300 text-sm font-medium hover:bg-zinc-700 transition-colors">Skip</button>
              </div>
            </div>
          </section>
        )}
            ...
            {gapResult ? (
              <>
                ...
                <div className="border-t border-zinc-800 pt-6 flex gap-3">
                  <button onClick={handleRewrite} disabled={loading} className="px-6 py-2.5 rounded-lg bg-emerald-700 text-white text-sm font-semibold hover:bg-emerald-600 disabled:opacity-50 disabled:cursor-not-allowed transition-colors">
              ...
              <div className="flex flex-col gap-4">
                <div>
                  ...
                  <p className="text-sm text-zinc-400 mt-1">No job description provided. Will optimize for general ATS best practices.</p>
                </div>
                <button onClick={handleRewrite} disabled={loading || !resumeText} className="self-start px-6 py-2.5 rounded-lg bg-emerald-700 text-white text-sm font-semibold hover:bg-emerald-600 disabled:opacity-50 disabled:cursor-not-allowed transition-colors">
                  {loading ? 'Rewriting...' : 'Rewrite Resume'}
                </button>
              </div>
            )}
