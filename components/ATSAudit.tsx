...
const STATUS_STYLES: Record<ATSCheck['status'], { badge: string; icon: string }> = {
  pass: {
    badge: 'bg-green-700/10 text-green-800 border border-green-700/25',
    icon: '✓',
  },
  fail: {
  ...
  unknown: {
    badge: 'bg-sunken text-stone ring-1 ring-seam',
    icon: '?',
  },
};
    ...
    <div className="flex flex-col gap-6">
      <div className="flex flex-col sm:flex-row items-center sm:items-start gap-6">
        ...
        <div className="flex flex-col gap-2 justify-center">
          <h2 className="text-xl font-semibold text-graphite">ATS Compatibility Score</h2>
      ...
      <div className="flex flex-col gap-2">
        {result.checks.map((check) => {
          ...
          return (
            <div
              key={check.id}
              className="flex items-start gap-3 p-3.5 rounded-[8px] bg-surface border border-hairline"
            >
              <span
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-0.5">
                  <span className="text-sm font-medium text-graphite">{check.name}</span>
                  <span
                    className={`w-1.5 h-1.5 rounded-full flex-shrink-0 ${IMPACT_DOT[check.impact]}`}
                    title={`${check.impact} impact`}
                  />
                </div>
                <p className="text-xs text-stone leading-relaxed">{check.message}</p>
              </div>
            </div>
          );
        })}
      ...
      <div className="flex items-center gap-3 text-xs text-stone">
        ...
        <span className="flex items-center gap-1.5">
          <span className="w-1.5 h-1.5 rounded-full bg-stone-400" /> Low impact
        </span>
      </div>
    </div>
  );
